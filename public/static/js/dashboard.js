...
const API_BASE_URL = 'https://medical-backend-9fjs.onrender.com';
...
function findHospitals(lat, lon) {
    fetch(`${API_BASE_URL}/api/find-hospitals`, {
        method: 'POST',
        credentials: 'include',
        headers: {
    ...
    const modalHTML = `
        <div id="language-modal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 10000; display: flex; align-items: center; justify-content: center;">
            <div style="background: white; padding: 40px; border-radius: 16px; max-width: 500px; text-align: center; box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
                <h2 style="margin: 0 0 24px 0; color: #1e293b;">🗣️ Choose Language / भाषा चुनें</h2>
...
function speakInLanguage(langCode, langName) {
    ...
    if (speakBtn) {
        ...
        speakBtn.innerHTML = `🔊 Speaking in ${langName}...`;
...
function getTranslatedSpeechText(data, langCode) {
    ...
    const templates = {
        ...
        'hi-IN': `आपका स्वास्थ्य विश्लेषण पूर्ण हो गया है। आपका जोखिम स्तर ${risk} है। ${hb !== null ? `हीमोग्लोबिन ${hb} ग्राम प्रति डेसीलीटर है। ` : ''}${bs !== null ? `रक्त शर्करा ${bs} मिलीग्राम प्रति डेसीलीटर है। ` : ''}${chol !== null ? `कोलेस्ट्रॉल ${chol} मिलीग्राम प्रति डेसीलीटर है। ` : ''}${risk === 'High' ? 'महत्वपूर्ण: कृपया तुरंत डॉक्टर से परामर्श लें।' : risk === 'Medium' ? 'हम अनुशंसा करते हैं कि आप डॉक्टर से जांच कराएं।' : 'स्वस्थ जीवनशैली बनाए रखें।'}`,
        
        'mr-IN': `तुमचे आरोग्य विश्लेषण पूर्ण झाले आहे। तुमचा जोखीम पातळी ${risk} आहे। ${hb !== null ? `हिमोग्लोबिन ${hb} ग्रॅम प्रति डेसीलीटर आहे। ` : ''}${bs !== null ? `रक्त साखर ${bs} मिलिग्रॅम प्रति डेसीलीटर आहे। ` : ''}${chol !== null ? `कोलेस्टेरॉल ${chol} मिलिग्रॅम प्रति डेसीलीटर आहे। ` : ''}${risk === 'High' ? 'महत्त्वाचे: कृपया लगेच डॉक्टरांचा सल्ला घ्या।' : risk === 'Medium' ? 'आम्ही शिफारस करतो की तुम्ही डॉक्टरांकडून तपासणी करा।' : 'निरोगी जीवनशैली चालू ठेवा।'}`,
        
        'ta-IN': `உங்கள் சுகாதார பகுப்பாய்வு முடிந்தது. உங்கள் ஆபத்து நிலை ${risk} ஆகும். ${hb !== null ? `ஹீமோகுளோபின் ${hb} கிராம் பெர் டெசிலிட்டர். ` : ''}${bs !== null ? `இரத்த சர்க்கரை ${bs} மில்லிகிராம் பெர் டெசிலிட்டர். ` : ''}${chol !== null ? `கொலஸ்டிரால் ${chol} மில்லிகிராம் பெர் டெசிலிட்டர். ` : ''}${risk === 'High' ? 'முக்கியமானது: உடனடியாக மருத்துவரை அணுகவும்।' : risk === 'Medium' ? 'மருத்துவரிடம் பரிசோதனை செய்யுமாறு பரிந்துரைக்கிறோம்।' : 'ஆரோக்கியமான வாழ்க்கை முறையை தொடரவும்।'}`,
        
        'te-IN': `మీ ఆరోగ్య విశ్లేషణ పూర్తయింది. మీ రిస్క్ స్థాయి ${risk}. ${hb !== null ? `హిమోగ్లోబిన్ ${hb} గ్రాములు పర్ డెసిలిటర్. ` : ''}${bs !== null ? `బ్లడ్ షుగర్ ${bs} మిల్లీగ్రాములు పర్ డెసిలిటర్. ` : ''}${chol !== null ? `కొలెస్ట్రాల్ ${chol} మిల్లీగ్రాములు పర్ డెసిలిటర్. ` : ''}${risk === 'High' ? 'ముఖ్యమైనది: దయచేసి వెంటనే వైద్యుడిని సంప్రదించండి।' : risk === 'Medium' ? 'వైద్యుడితో చెకప్ చేయించుకోవాలని సిఫారసు చేస్తున్నాము।' : 'ఆరోగ్యకరమైన జీవనశైలిని కొనసాగించండి।'}`,
        
        'bn-IN': `আপনার স্বাস্থ্য বিশ্লেষণ সম্পূর্ণ হয়েছে। আপনার ঝুঁকির মাত্রা ${risk}। ${hb !== null ? `হিমোগ্লোবিন ${hb} গ্রাম প্রতি ডেসিলিটার। ` : ''}${bs !== null ? `রক্তের শর্করা ${bs} মিলিগ্রাম প্রতি ডেসিলিটার। ` : ''}${chol !== null ? `কোলেস্টেরল ${chol} মিলিগ্রাম প্রতি ডেসিলিটার। ` : ''}${risk === 'High' ? 'গুরুত্বপূর্ণ: অনুগ্রহ করে অবিলম্বে ডাক্তারের পরামর্শ নিন।' : risk === 'Medium' ? 'আমরা সুপারিশ করি যে আপনি ডাক্তারের কাছে চেকআপ করান।' : 'স্বাস্থ্যকর জীবনযাত্রা বজায় রাখুন।'}`,
        
        'gu-IN': `તમારું આરોગ્ય વિશ્લેષણ પૂર્ણ થયું છે। તમારું જોખમ સ્તર ${risk} છે। ${hb !== null ? `હિમોગ્લોબિન ${hb} ગ્રામ પ્રતિ ડેસિલિટર છે। ` : ''}${bs !== null ? `બ્લડ સુગર ${bs} મિલિગ્રામ પ્રતિ ડેસિલિટર છે। ` : ''}${chol !== null ? `કોલેસ્ટ્રોલ ${chol} મિલિગ્રામ પ્રતિ ડેસિલિટર છે। ` : ''}${risk === 'High' ? 'મહત્વપૂર્ણ: કૃપા કરીને તાત્કાલિક ડૉક્ટરની સલાહ લો।' : risk === 'Medium' ? 'અમે ભલામણ કરીએ છીએ કે તમે ડૉક્ટર પાસે તપાસ કરાવો।' : 'સ્વસ્થ જીવનશૈલી જાળવી રાખો।'}`,
        
        'kn-IN': `ನಿಮ್ಮ ಆರೋಗ್ಯ ವಿಶ್ಲೇಷಣೆ ಪೂರ್ಣಗೊಂಡಿದೆ। ನಿಮ್ಮ ಅಪಾಯದ ಮಟ್ಟ ${risk} ಆಗಿದೆ। ${hb !== null ? `ಹಿಮೋಗ್ಲೋಬಿನ್ ${hb} ಗ್ರಾಂ ಪ್ರತಿ ಡೆಸಿಲಿಟರ್। ` : ''}${bs !== null ? `ರಕ್ತದ ಸಕ್ಕರೆ ${bs} ಮಿಲಿಗ್ರಾಂ ಪ್ರತಿ ಡೆಸಿಲಿಟರ್। ` : ''}${chol !== null ? `ಕೊಲೆಸ್ಟರಾಲ್ ${chol} ಮಿಲಿಗ್ರಾಂ ಪ್ರತಿ ಡೆಸಿಲಿಟರ್। ` : ''}${risk === 'High' ? 'ಮುಖ್ಯವಾದುದು: ದಯವಿಟ್ಟು ತಕ್ಷಣ ವೈದ್ಯರನ್ನು ಸಂಪರ್ಕಿಸಿ।' : risk === 'Medium' ? 'ವೈದ್ಯರೊಂದಿಗೆ ತಪಾಸಣೆ ಮಾಡಿಸಿಕೊಳ್ಳುವಂತೆ ನಾವು ಶಿಫಾರಸು ಮಾಡುತ್ತೇವೆ।' : 'ಆರೋಗ್ಯಕರ ಜೀವನಶೈಲಿಯನ್ನು ಮುಂದುವರಿಸಿ।'}`
    };
